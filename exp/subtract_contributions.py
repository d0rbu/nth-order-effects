...
def main(
    *args,
    ...
    sorted_delta_losses = sorted(delta_losses, key=lambda x: x.relative_loss)
    ...
    plt.bar(range(len(sorted_delta_losses)), [x.relative_loss for x in sorted_delta_losses], tick_label=[str(x.unit_indices) for x in sorted_delta_losses])
